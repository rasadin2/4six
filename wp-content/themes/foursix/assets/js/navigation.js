...
(function($) {
    ...
    var foursixMenu = $('.foursix-inline-menu');
    $('.foursix-inline-menu li').find('> .sub-menu').before('<span class="mb-toggle js-show-mb-nav"><i class="fas fa-chevron-down"></i></span>'); // Desktop Menu

    var desktopMenu = function desktopMenu() {
        if ($(window).width() > screenWidth) {
            var foursixMenuLink = $('.foursix-inline-menu li');
            foursixMenuLink.find('> ul.sub-menu').addClass('hide');
            foursixMenuLink.on('mouseover', function() {
                $(this).find('> ul.sub-menu').addClass('open').removeClass('hide').stop(true, true);
            });
            foursixMenuLink.on('mouseleave', function() {
